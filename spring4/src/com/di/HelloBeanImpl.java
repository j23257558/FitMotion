 ...
 *  
 * IoC(Inversion of Conversion) Container임 - 스프링은 역제어 라고함 
 * ---------------기존방식 ------------------------
 * 자바기반으로 어플리케이션을 개발 할때 자바 객체를 생성하고 서로간의 의존관계를 연결시키는 
 * 작업에 대한 제어권을 보통개발되는 어플리케이션에 있음.
 * 문제제기 ---------------------------------------
 * 컴포넌트 간의 결합도가 높아서 컴포넌트의 확장및 재사용이 어려운 문제점이 발생됨.
 * IoC 사용시 -----------------------------------------
 * 제어권이 Container에게 넘어가 객체의 생명주기를 컨테이너가 전담하게 됨.
 * 예)Sevlet, EJB
 * 장점 : 컴포넌트간의 결합도가 낮아져 컴포넌트의 재사용 및 확장이 쉽고 컴포넌트의 의존관계 변경이 쉬움.
 * IoC에서 이야기하는 제어권의 역전이란?
 * 객체의 생성 및 생명주기에 대한 제어권이 개발자에서 Container로 넘어갔다는 것을 의미.
 * 
 * 스프링 컨테이너의 유형 
 * Spring Container는 BeanFactory와 ApplicationContext라는 두가지 유형의
 * 컨테이너를 제공함 
 * BeanFactory
 * 객체를 관리하는 고급 설정 기반임.
 * Bean들에 대한 생성, 소멸, 라이프 사이클과 같이 컨테이너가 빈을 관리하기 위한
 * 필요한 기능 제공함.
 * XmlBeanFactory
